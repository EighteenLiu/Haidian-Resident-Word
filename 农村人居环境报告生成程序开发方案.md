...
提交 xlsx 的 `数据明细` 按当前辅助拼接列匹配口径只输出有效案件明细；6 月样例源数据当前纳入 664 条有效案件。样例第 1 行为表头，没有需要保留的尾部空白格式行。
...
| 控件 | 行为 |
| --- | --- |
| `生成台账` 按钮 | 在 `生成台账` 模块中读取主数据、字典、道路台账，完成筛选、道路匹配、统计，并生成 xlsx 台账 |
| `应用于报告生成` 按钮 | 台账生成后启用，把生成的 xlsx 路径带入 `生成报告` 模块并跳转页面，用户可先人工修改台账再继续 |
| `更新统计表并生成报告` 按钮 | 在 `生成报告` 模块中读取所选台账 xlsx 的 `数据明细`，重新统计并同步 `sheet1`，再生成 Word |
...
flowchart TD
    ...
    L --> M["从台账数据明细重新统计"]
    M --> N["同步更新台账 sheet1"]
    N --> O["构建 docx Jinja 上下文"]
    O --> P["渲染 Word 报告"]
    P --> Q["后处理合并 Word 表格单元格"]
    Q --> R["输出文件和日志"]
```
...
道路匹配只使用道路台账 `备注-别名` 后一列的辅助拼接值，不再用 `道路` 列或 `备注-别名` 列本身作为正式纳入口径。

道路名称按括号情况生成辅助拼接候选：源道路名无括号时，生成 `检查点位（3级）+道路原值`；源道路名带括号时，生成 `检查点位（3级）+括号前名称`、`检查点位（3级）+括号内名称` 和 `检查点位（3级）+带括号完整道路名`。任意候选命中道路台账 `备注-别名` 后一列的辅助拼接值，即纳入统计。
...
也就是说：先做道路匹配和校验，未命中道路台账辅助拼接列的记录不纳入正式统计；道路为空且不是 `考评上报` 的记录按当前统计口径纳入。
...
排序按问题总数从高到低，数量相同再按参考报告或字典顺序，避免数量变化后镇顺序乱跳。
...
表 4 按镇分组，镇内按 `total_count` 从高到低排序。
...
生成 `台账生成日志.txt` 和 `报告生成日志.txt`，内容包括：
...
| `test_data_filter.py` | 6 月样例按辅助拼接列匹配口径能筛出 664 条有效案件 |
...
```python
def on_generate_ledger_clicked():
    disable_controls()
    start_time = time.perf_counter()
    worker = threading.Thread(target=generate_ledger_worker, daemon=True)
    worker.start()

def on_generate_report_clicked():
    disable_controls()
    worker = threading.Thread(target=generate_report_from_ledger_worker, daemon=True)
    worker.start()
```
...
queue.put(("ledger_done", ledger_result))
queue.put(("report_done", report_result))
