...
提交 xlsx 的 `数据明细` 按样例只输出有效案件明细。样例是第 1 行表头、第 2-664 行 663 条有效案件，没有需要保留的尾部空白格式行。
...
也就是说：先做道路匹配和校验，但默认不静默剔除未命中记录。等用户确认“未命中道路是否剔除”后再改为 `drop_unmatched`。
...
| `test_data_filter.py` | 6 月样例能筛出 663 条有效案件 |
| `test_road_matcher.py` | 标准道路、别名道路、空道路、未命中道路分类正确 |
| `test_statistics_builder.py` | 表 1 总计 663，各大类数量与样例一致 |
