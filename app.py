...
class ResidentReportApp(ctk.CTk):
    ...
    def _build_ui(self) -> None:
        ...
        self.source_frame.grid(row=0, column=0, sticky="ew", **pad)
        self.source_frame.grid_columnconfigure(1, weight=1)
        ctk.CTkLabel(self.source_frame, text="源文件上传").grid(row=0, column=0, sticky="w", padx=10, pady=8)
        self.choose_source_btn = ctk.CTkButton(self.source_frame, text="选择源文件", command=self.choose_sources)
        self.choose_source_btn.grid(row=0, column=1, sticky="w", padx=8)
        self.clear_source_btn = ctk.CTkButton(self.source_frame, text="清空", width=80, command=self.clear_sources)
        self.clear_source_btn.grid(row=0, column=2, sticky="e", padx=8)
        self.source_text = ctk.CTkTextbox(self.source_frame, height=96)
        self.source_text.grid(row=1, column=0, columnspan=3, sticky="ew", padx=10, pady=(0, 10))

        self.template_frame = ctk.CTkFrame(self)
        self.template_frame.grid(row=1, column=0, sticky="ew", **pad)
        self.template_frame.grid_columnconfigure(1, weight=1)
        ctk.CTkLabel(self.template_frame, text="模板文件").grid(row=0, column=0, sticky="w", padx=10, pady=8)
        self.word_label = ctk.CTkLabel(self.template_frame, text="", anchor="w")
        self.word_label.grid(row=1, column=1, sticky="ew", padx=8)
        self.xlsx_label = ctk.CTkLabel(self.template_frame, text="", anchor="w")
        self.xlsx_label.grid(row=2, column=1, sticky="ew", padx=8)
        ctk.CTkButton(self.template_frame, text="选择 Word 模板", command=self.choose_word_template).grid(row=1, column=0, padx=10, pady=4)
        ctk.CTkButton(self.template_frame, text="选择 xlsx 模板", command=self.choose_xlsx_template).grid(row=2, column=0, padx=10, pady=4)

        self.date_frame = ctk.CTkFrame(self)
        self.date_frame.grid(row=2, column=0, sticky="ew", **pad)
        for i in range(10):
        ...
        self.output_frame.grid(row=3, column=0, sticky="ew", **pad)
        ...
        self.control_frame.grid(row=4, column=0, sticky="ew", **pad)
        ...
        self.log_box.grid(row=5, column=0, sticky="nsew", **pad)

    def _refresh_template_labels(self) -> None:
        self.word_label.configure(text=str(self.word_template) if self.word_template else "未找到模板，请手动选择模板文件。")
        self.xlsx_label.configure(text=str(self.xlsx_template) if self.xlsx_template else "未找到模板，请手动选择模板文件。")
    ...
    def refresh_source_list(self) -> None:
        self.source_text.delete("1.0", "end")
        classified = classify_files(self.source_files)
        for item in classified:
            self.source_text.insert("end", f"{item.path.name} - {item.file_type.value} - {item.reason}\n")
        main = next((item for item in classified if item.file_type.value == "主数据"), None)
        if main:
            try:
                opts = parse_dates_from_main_file(main.path, main.sheet_name)
                self.year_var.set(str(opts.report_year))
                self.month_var.set(str(opts.report_month))
                self.start_var.set(opts.start_date.isoformat() if opts.start_date else "")
                self.end_var.set(opts.end_date.isoformat() if opts.end_date else "")
